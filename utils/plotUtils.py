...
def gradientbarsHor(bars, ax, maxwidth, barpad):
    ...
    for bar in bars:
        ...
        x,y = bar.get_xy()
        w, h = bar.get_width(), bar.get_height()
        ax.imshow(grad, extent=[x,x+w,y,y+h], aspect="auto", cmap='gray', zorder=0, alpha=1)
        ax.imshow(residual_grad, extent=[x+w, maxwidth+barpad, y, y+h], aspect="auto", cmap='gray_r', alpha=.3, zorder=0)
